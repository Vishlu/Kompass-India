...
def read_excel(file_path):
    ...
    # Apply strip() function to each string value in DataFrame, skip NaN (float) values
    df = df.apply(lambda x: x.strip() if isinstance(x, str) else x)
...
def update_comp_json(row,company_json,output_json_file):
    ...
    try:
        ...
        if(
            ...
            # If any executive properties are present then proceed
            if 'executives' in company_json:
                # Storing exec info for comparison
                executives = company_json.get('executives', [])
                exec_first_name_list = [exec.get('firstName', '') for exec in executives]
                exec_last_name_list = [exec.get('lastName', '') for exec in executives]
                exec_first_name_list = [name.split()[0] if len(name.split()) > 1 else name for name in exec_first_name_list]
                exec_last_name_list = [name.split()[-1] if len(name.split()) > 1 else name for name in exec_last_name_list]
                # print('First Names List :',exec_first_name_list)
                # print('Last Names List :',exec_last_name_list)
                
                exec_type = 'executive'
                designation = str(row.get('functionName'))
                if(designation=='Chairman' or designation=='Director' or str(row.get('functionCode')).strip()=='5101' or str(row.get('functionCode')).strip()=='5102'): 
                    exec_type = 'board'

                # If a completely new executive is to be inserted
                if (str(row.get('firstName')).split() or [''])[0] not in exec_first_name_list or (str(row.get('lastname')).split() or [''])[-1] not in exec_last_name_list:
                    # exec_func_key = str(row.get('functionName').strip())
                    #print('Current Designation :',row.get('functionName'))
                    main_function = str(int(row.get('functionCode'))).strip()
                    #print('Current Mainfunction Code :',main_function)
                    exec_info = {
                        'isoCode': 'en',
                        "executiveType": exec_type,
                        "mainfunction": str(main_function),
                        "salutation": str(row.get('salutation')) if pd.notna(row.get('salutation')) else '',
                        "firstName": (row.get('firstName').split() or [''])[0] if pd.notna(row.get('firstName')) else '',
                        "lastName": (row.get('lastname').split() or [''])[-1] if pd.notna(row.get('lastname')) else '',
                        "functionName": str(row.get('functionName')).strip() if pd.notna(row.get('functionName')) else '',
                        "emailAddress": row.get('executiveEmail') if pd.notna(row.get('executiveEmail')) else '',
                        'order' : 0,
                        'source' : source_variable,
                        'mobilePhoneNumber': {
                            "countryPhoneCode": str(int(row.get('ExecCountryCode'))) if pd.notna(row.get('ExecCountryCode')) else '',
                            "number": str(int(row.get('ExecMobileNumber'))) if pd.notna(row.get('ExecMobileNumber')) else ''
                        },
                        'socialNetworks': [{'address': row.get('linkedinprofile') if pd.notna(row.get('linkedinprofile')) else ''}],
                    }

                    # Append the exec_info dictionary to the 'executives' list
                    company_json['executives'].append(exec_info)
                    executives_with_order = [executive for executive in company_json.get('executives', []) if 'order' in executive]
                    executives_without_order = [executive for executive in company_json.get('executives', []) if 'order' not in executive]
                    sorted_executives = sorted(executives_with_order, key=lambda x: (x.get('mainfunction', '')), reverse=False) + executives_without_order
                    order_counter = 1
                    for executive in sorted_executives:
                        executive['order'] = order_counter
                        order_counter += 1
                    # Update the 'executives' list in the company_json
                    company_json['executives'] = sorted_executives
                    ...
                    # If the executive already exists, update its information
                    match_found = False

                    for person_info in executives:
                        f_n = (person_info.get('firstName', '').split() or [''])[0] 
                        l_n = (person_info.get('lastName', '').split() or [''])[-1]
                        # print('First Name :',f_n,end=' ')
                        # print('Last Name :',l_n,end=' ')
                        func_name = person_info.get('functionName', '').strip()
                        social_networks = person_info.get("socialNetworks", [])
                        addresses = [entry.get("address", "") for entry in social_networks]

                        if (
                            ((pd.notna(row['firstName']) and row['firstName'].split() or [''])[0] == f_n) and
                            ((pd.notna(row['lastname']) and row['lastname'].split() or [''])[-1] == l_n)
                        ):
                            # If a match is found, update the existing person_info
                            match_found = True
                            person_info['source'] = source_variable
                            if pd.notna(row['firstName']):
                                person_info['firstName'] = (row['firstName'].split() or [''])[0]
                            if pd.notna(row['lastname']):
                                person_info['lastName'] = (row['lastname'].split() or [''])[-1]
                            if pd.notna(row['functionName']):
                                person_info['functionName'] = row['functionName'].strip()
                                person_info['executiveType'] = exec_type 
                            if pd.notna(row['functionCode']):
                                person_info['mainfunction'] = str(int(row.get('functionCode')))

                            if pd.notna(row['executiveEmail']):
                                person_info['emailAddress'] = row['executiveEmail']
                            if pd.notna(row['salutation']):
                                person_info['salutation'] = row['salutation']
                            # if pd.notna(row['ExecCountryCode']):
                            #     person_info['mobilePhoneNumber']['countryPhoneCode'] = str(int(row['ExecCountryCode']))
                            #     # print('Type of Country Code :',type(person_info['mobilePhoneNumber']['countryPhoneCode']))
                            if pd.notna(row['ExecMobileNumber']):
                                
                                if 'mobilePhoneNumber' not in person_info:
                                    person_info['mobilePhoneNumber'] = {}
                                    person_info['mobilePhoneNumber']['number'] = str(int(row['ExecMobileNumber']))
                            ...
                            if pd.notna(row['linkedinprofile']):
                                if(row['linkedinprofile'] not in addresses):
                                    person_info['socialNetworks'].append({"address": row['linkedinprofile']})


                    # Sort the executives based on 'mainfunction' and 'order'
                    executives_with_order = [executive for executive in company_json.get('executives', []) if 'order' in executive]
                    executives_without_order = [executive for executive in company_json.get('executives', []) if 'order' not in executive]
                    sorted_executives = sorted(executives_with_order, key=lambda x: (x.get('mainfunction', '')), reverse=False) + executives_without_order
                ...
                for executive in company_json['executives']:
                    ...
                    # Check if the combination is not seen before
                    if key not in seen_combinations:
                        unique_executives.append(executive)
                        seen_combinations.add(key)

                #print('Unique Execs are :',unique_executives)
                company_json['executives'] = unique_executives
                executives_with_order = [executive for executive in company_json.get('executives', []) if 'mainfunction' in executive and executive['mainfunction']]
                executives_without_order = [executive for executive in company_json.get('executives', []) if executive.get('mainfunction', '') == ""]
                #print('Executives with order :',executives_with_order)
                #print('Executives without order :',executives_without_order)
                sorted_executives = sorted(executives_with_order, key=lambda x: (x.get('mainfunction', '')), reverse=False) + executives_without_order
                # Update the 'order' field
                order_counter = 1
                for executive in sorted_executives:
                ...
                # If 'executives' list is not present, create a new list
                if (
                    pd.notna(row.get('salutation')) or pd.notna(row.get('firstName')) or
                    pd.notna(row.get('lastname')) or pd.notna(row.get('functionName')) or
                    pd.notna(row.get('executiveEmail')) or pd.notna(row.get('ExecCountryCode')) or
                    pd.notna(row.get('ExecMobileNumber')) or pd.notna(row.get('linkedinprofile'))
                ):
                    exec_func_key = row.get('functionName').strip()
                    main_function = str(int(row.get('functionCode'))).strip()
                    exec_type = 'executive'
                    designation = str(row.get('functionName'))
                    if(designation=='Chairman' or designation=='Director' or str(row.get('functionCode')).strip()=='5101' or str(row.get('functionCode')).strip()=='5102'): 
                    ...
                    exec_info = {
                        ...
                        "firstName": row.get('firstName') if pd.notna(row.get('firstName')) else '',
                        "lastName": row.get('lastname') if pd.notna(row.get('lastname')) else '',
                    ...
                    # Create a new 'executives' list with the exec_info dictionary
                    company_json['executives'] = [exec_info]

                    company_json['executives'] = sorted(company_json.get('executives', []), key=lambda x: (x.get('mainfunction', '')), reverse=False)
                    order_counter = 1
                    for executive in company_json['executives']:
                        executive['order'] = order_counter
                        order_counter += 1
    ...
    # Logic for turnover
        if(pd.notna(row.get('turnover'))):
            ...
            if 'financialDatas' in company_json:
                ...
                #     'Over 50000 Million': 'J'
                # }
                excel_turnover = str(row.get('turnover')).strip() 
...
def main():
    excel_file_path = r"C:\Users\Kompass\Desktop\Test Files\Salutation_AddNewCompany_01.03.2025.xlsx"
    ...
    for index, row in tqdm(excel_file.iterrows(),desc='Progress Bar : ',total=total_rows):
        ...
        try:
            ...
            print()
            print(f'New Company created : {company_id}')
            print()
        except Exception as e:
